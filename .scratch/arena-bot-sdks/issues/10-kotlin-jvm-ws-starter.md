...
A **Kotlin on the JVM** WS starter so Kotlin users can compete (Kotlin/Wasm isn't supported in v1,
so Kotlin takes the WS path). It connects over WebSocket, performs the welcome → join(token) →
...
- [ ] The starter connects over WebSocket on the JVM and completes the welcome → join(token) →
      assigned handshake.
...
- [ ] Observation/action schema round-trip is tested; build/run/submit docs are included.
