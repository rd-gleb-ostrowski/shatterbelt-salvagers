...
A **TypeScript on Deno** WS starter so participants can write a bot in TypeScript on a modern
runtime. It connects over WebSocket, performs the welcome → join(token) → assigned handshake, then
loops reading `tick` observation JSON and writing `action` intent JSON, exposing a tiny
`tick(observation) -> action` surface.
...
- [ ] The starter connects over WebSocket on Deno and completes the welcome → join(token) →
      assigned handshake.
